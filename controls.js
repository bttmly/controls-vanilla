...
(function() {
  var $_, ControlCollection, Factory, changeEvent, controlValidations, each, elClear, elValid, elValue, eventId, every, extend, filter, find, invalidEvent, isFunction, makeEvent, map, p, removeWhere, slice, trigger, validEvent,
  ...
  trigger = function(target, evt) {
    var method;
    if (!(evt instanceof Event)) {
      throw new TypeError("Pass an event!");
    }
  ...
  validEvent = function() {
    return makeEvent("valid");
  };

  invalidEvent = function() {
    return makeEvent("invalid");
  };

  changeEvent = function() {
    return makeEvent("change");
  };
  ...
  ControlCollection = (function(_super) {
    ...
    ControlCollection.prototype.on = function(eventType, handler) {
      ...
      if (eventType === "valid" || eventType === "invalid") {
        this.setValidityListener();
      }
      eventHandler = (function(event) {
        var id, listener, _ref, _ref1;
        if (_ref = event.target, __indexOf.call(this, _ref) >= 0) {
          if ((id = (_ref1 = event.detail) != null ? _ref1.id : void 0)) {
            listener = find(this._eventListeners[eventType], function(l) {
              return l.handler = eventHandler;
            });
            if (listener && (__indexOf.call(listener.eventIds, id) >= 0)) {
    ...
    ControlCollection.prototype.off = function(eventType, handler) {
      removeWhere(this._eventListeners[eventType] || [], function(o) {
