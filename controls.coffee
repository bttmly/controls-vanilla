...
# In-place removal of elements matching test.
removeWhere = ( arr, test ) ->
  i = 0
  while i < arr.length
    if test( arr[i] )
      arr.splice( i, 1 )
...
trigger = ( target, evt ) ->
  throw new TypeError( "Pass an event!" ) unless evt instanceof Event
  # evt = makeEvent( evt ) unless evt instanceof Event
...
class ControlCollection extends Array
  ...
  on: ( eventType, handler ) ->
    @setValidityListener() if eventType is "valid" or eventType is "invalid"
    
    eventHandler = ( ( event ) ->
      if event.target in this
        if ( id = event.detail?.id )
          listener = find @_eventListeners[ eventType ], ( l ) -> l.handler = eventHandler
          if listener and ( id in listener.eventIds )
            console.log listener
            return 
          listener.eventIds.push( id )
        handler.call( this, event )
    ).bind( this )

    document.addEventListener( eventType, eventHandler )
    @_eventListeners[ eventType ] or= []
    @_eventListeners[ eventType ].push( { handler: eventHandler, eventIds: [] } )
    eventHandler

  # Remove a previously attached event listener.
  off: ( eventType, handler ) ->
    removeWhere ( @_eventListeners[ eventType ] or [] ), ( o ) -> o.handler is handler
  ...
  trigger: ( evt ) ->
    evt = makeEvent( evt ) unless evt instanceof Event
    trigger( control, evt ) for control in this
    this
