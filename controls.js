...
(function() {
  var ControlCollection, Factory, changedEvent, controlValidations, each, elClear, elValid, elValue, filter, map, p, slice, validEvent,
  ...
  controlValidations = {
    ...
    numberBetween: function(min, max) {
      return function(el) {
        var _ref;
        return (min <= (_ref = Number(el.value)) && _ref <= max);
      };
    },
    numberMax: function(max) {
      return controlValidations.numberBetween(0, max);
    },
    numberMin: function(min) {
      return controlValidations.numberBetween(min, Number.POSITIVE_INFINITY);
    },
    lengthBetween: function(min, max) {
      return function(el) {
        var _ref;
        return (min <= (_ref = el.value.length) && _ref <= max);
      };
    },
  ...
  ControlCollection = (function(_super) {
    ...
    ControlCollection.prototype.on = function(eventType, handler) {
      ...
      return document.addEventListener(eventType, eventHandler);
    };
  ...
  Factory = (function() {
    ...
    return function(parentSelector) {
      var controls, parent;
      parent = document.querySelector(parentSelector);
      controls = parent.querySelectorAll(controlTags.join(", "));
      return new ControlCollection(controls);
    };
  })();
